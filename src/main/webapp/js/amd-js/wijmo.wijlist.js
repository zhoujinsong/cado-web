...
(function (wijmo) {
    ...
    */
    (function (_list) {
        ...
        var listItemCSS = "wijmo-wijlist-item", listItemCSSAlternate = listItemCSS + "-alternate", listItemCSSSelected = listItemCSS + "-selected", listItemCSSFirst = listItemCSS + "-first", listItemCSSLast = listItemCSS + "-last", activeItem = "wijmo-wijlistitem-active", itemKey = "item.wijlist";
        ...
        var wijlist = (function (_super) {
            ...
            */
            wijlist.prototype.addItem = function (item, index) {
                ...
                self._trigger("added", null, { item: item, index: index });
            };
                    ...
                    $.each(this.items, function (index, item) {
                        if (item.selected) {
                            ...
                            if (item.element && item.element.hasAllClasses(selectedActive)) {
                                item.element.removeClass(selectedActive).removeClass("wijmo-wijcombobox-selecteditem");
                            }
                        }
                    });
                    return;
            ...
            */
            wijlist.prototype.getSelectedItems = function () {
            ...
            wijlist.prototype.select = function (event, data) {
                ...
                if (singleMode) {
                    ...
                    if (item.selected) {
                        ...
                        /*
                        Because the tap behavior will also trigger the mouseover event in touch device.
                        And only when other element catch the mouseover event, current item will trigger the mouse out event.
                        So when tapping the selected list item to unselect, the hover state style will be retained on the list item.
                        Removing the hover state status when unselect the list item for touch device.
                        */
                        if ($.support.isTouchEnabled && $.support.isTouchEnabled()) {
                            ele.removeClass(o.wijCSS.stateHover);
                        }
                    }
                    selectedIndex = [];
            ...
            */
            wijlist.prototype.unselectItems = function (indices) {
                ...
                if (mode === "single") {
                    ...
                    if (selectedItem) {
                        selectedItem.selected = false;
                        selectedItem.element.removeClass(selectedActive).removeClass("wijmo-wijcombobox-selecteditem");
                        self.selectedItem = undefined;
                    }
                } else {
                    ...
                    $.each(foundItems, function (index, i) {
                        i.selected = false;
                        i.element.removeClass(selectedActive).removeClass("wijmo-wijcombobox-selecteditem");
                    });
                    self.selectedItems = $.grep(self.items, function (a) {
                        return a.selected;
                    }, undefined);
            ...
            /** @ignore*/
            wijlist.prototype.adjustOptions = function () {
        ...
        ;
        wijlist.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, new wijlist_options());
