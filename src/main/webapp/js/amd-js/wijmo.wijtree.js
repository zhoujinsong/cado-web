...
(function (wijmo) {
    ...
    */
    (function (_tree) {
        "use strict";
        var $ = jQuery, widgetName = "wijtree", nodeWidgetName = "wijtreenode", generateItemsMarkup = function (items, showExpandCollapse) {
            ...
            $.each(items, function (i, item) {
                ...
                var u = item.navigateUrl || "#", hasChildren = item.nodes && item.nodes.length > 0, expanded = (item.expanded === null || item.expanded === undefined) ? opts.expanded : item.expanded, wijCSS = (item.wijCSS === null || item.wijCSS === undefined) ? opts.wijCSS : item.wijCSS, li = "<li class='" + (hasChildren ? "wijmo-wijtree-parent" : "wijmo-wijtree-item") + "'><div class='wijmo-wijtree-node " + wijCSS.stateDefault + (hasChildren ? " wijmo-wijtree-header " : "") + (showExpandCollapse && hasChildren ? (expanded ? "wijmo-state-expanded" : "wijmo-state-collapsed") : "") + "' role='treeitem' aria-expanded='false' aria-checked='false' aria-selected='false'><span class='wijmo-wijtree-inner " + wijCSS.helperClearFix + " " + wijCSS.cornerAll + "'>";
                if (hasChildren) {
                ...
                if (hasChildren) {
                    ...
                    li += ">";
                    li += generateItemsMarkup(item.nodes, showExpandCollapse);
                    li += "</ul>";
                }
                li += "</li> ";
                lis += li;
        ...
        var wijtree = (function (_super) {
            ...
            wijtree.prototype._createChildNodes = function () {
                ...
                if (!self._useDOMInit || data.length) {
                    ns.empty();
                    lis = generateItemsMarkup(data, self.options.showExpandCollapse);
                    ns.html(lis);
                    ...
                    lis.each(function (idx, ele) {
                        ...
                        data.push(widget.options);
                    });
                }

                this.widgetDom.append(ns);
            ...
            */
            wijtree.prototype.add = function (node, position) {
                var nodeWidget = null, o = {}, $node, nodes, self = this, i, originalLength, itemDom, cnodes;
                if ($.browser.safari) {
            ...
            */
            ...
            wijtree.prototype._setAllowDrag = function (value) {
                var self = this, $allNodes, nodeSelector = self._nodeSelector(), nodeWidgetName = "wijmo-" + self.nodeWidgetName;
                ...
                if (value) {
                    ...
                    $allNodes.each(function () {
                        ...
                        if (!$(this).data("ui-draggable") && !w.$navigateUrl.data("events") || !w.$navigateUrl.data("events").mousedown) {
        ...
        var wijtreenode = (function (_super) {
            ...
            wijtreenode.prototype._createTreeNode = function () {
                ...
                if (self._isHtmlGenerated()) {
                    ...
                    if (!$.browser.safari) {
                        self.$navigateUrl.addClass("wijmo-wijtree-link");
                    }
                } else {
                    self.$nodeImage = $("<span>");
            ...
            wijtreenode.prototype._isHtmlGenerated = function () {
                if (this.htmlGenerated) {
                    return true;
                }
                if (this._tree && this._tree.htmlGenerated) {
                    return true;
                }
                return false;
            };
                ...
                if (o.nodes && o.nodes.length) {
                    if (self._isHtmlGenerated()) {
                        ...
                        $.each(self.$subnodes, function (idx, $li) {
                            ...
                            opts.nIndex = idx;
                            widget = createTreeNodeWidget($li, opts, self, nodeWidgetName, self.options.treeClass);
                            widget._index = idx;
                            nodes.push(widget);
                        });
                    } else {
                        ...
                        self.$nodes.html(generateItemsMarkup(o.nodes, self._tree.options.showExpandCollapse));
                        self.$subnodes = [];
                        self.htmlGenerated = true;
            ...
            /*behavior Methods*/
            wijtreenode.prototype._expandCollapseItem = function () {
                var self = this, o = self.options;
            ...
            wijtreenode.prototype._animation = function (show) {
                ...
                if (el && el.length) {
                    if (animation) {
                        ...
                        if ($.effects && !!effect) {
                            el[show ? "show" : "hide"](effect, { easing: animation.easing }, animation.duration, function () {
                                restoreOpacity(el);
            ...
            wijtreenode.prototype._beginDrag = function (e) {
                ...
                options.drag = function (event, ui) {
                    var t = event.srcElement || event.originalEvent.target, targetEl = $(t), dropNode, p = { x: event.pageX, y: event.pageY };
                    if (temp) {
                        temp.hide();
                    }
                    if (targetEl) {
            ...
            wijtreenode.prototype._resetDrag = function () {
                ...
                if (!self._tree.options.allowDrag && self.element.data("ui-draggable")) {
            ...
            */
            wijtreenode.prototype.add = function (node, position) {
                ...
                if (!$.browser.safari) {
                    link = self.element.children(".wijmo-wijtree-node").find(".wijmo-wijtree-inner .wijmo-wijtree-link");
                    if (link && link.length > 0) {
                        link.removeClass("wijmo-wijtree-link");
                    }

                    checkbox = self.element.children(".wijmo-wijtree-node").find(".wijmo-wijtree-inner .wijmo-checkbox");
        ...
        var wijtreecheck = (function (_super) {
            ...
            wijtreecheck.prototype._create = function () {
                ...
                if (self.element.is("div")) {
                    ...
                    self.$body = $('<div></div>').addClass(["wijmo-checkbox-box", css.widget, css.cornerAll, css.stateDefault].join(' ')).css({ position: "relative" }).append(self.$icon);
        ...
        _tree.wijtreecheck = wijtreecheck;

        wijtreecheck.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, {
            wijMobileCSS: {
