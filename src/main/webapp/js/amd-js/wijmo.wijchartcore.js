 ...
 */
var wijmo;
define(["./wijmo.widget", "./wijmo.raphael", "./wijmo.wijtooltip", "globalize"], function () { 
    ...
    (function (chart) {
        ...
        var TrendlineRender = (function () {
            ...
            TrendlineRender.prototype.render = function () {
                var element = this.element, options = this.options, wijCSS = options.wijCSS, cBounds = options.bounds, widgetName = options.widgetName, canvas = options.canvas, ani = options.animation, seTrans = options.seriesTransition, mouseDown = options.mouseDown, mouseUp = options.mouseUp, mouseOver = options.mouseOver, mouseOut = options.mouseOut, mouseMove = options.mouseMove, click = options.click, linesStyle = [], paths = [], shadowPaths = [], disabled = options.disabled, animationSet = canvas.set(), fieldsAniPathAttr = options.aniPathsAttr, aniPathsAttr = [], chartEles, fields = element.data("fields") || {}, seriesEles = [], widget = options.widget, exVal = options.extremeValue, clipRect = {
                    enable: false,
                    left: -10,
                    top: -10,
                    right: 10,
                ...
                if (ani.enabled || seTrans.enabled) {
                    TrendlineRender.playAnimation(ani.duration, ani.easing, seTrans, animationSet, cBounds, paths, fieldsAniPathAttr, clipRect, options.axis, exVal);
                } else if (clipRect.enable) {
                    TrendlineRender.setClipRect(clipRect, animationSet, cBounds);
                }
            ...
            TrendlineRender.setClipRect = function (clipRect, animationSet, cBounds) {
            ...
            TrendlineRender.playAnimation = function (duration, easing, seTrans, animationSet, cBounds, paths, fieldsAniPathAttr, clipRect, axis, exVal) {
                if (!clipRect) {
                    clipRect = {
                        enable: false,
                        left: -10,
                    ...
                    if (!axis.y.autoMax && axis.y.max < exVal.tyx) {
                        clipRect.enable = true;
                        clipRect.top = 0;
                    }
                    if (!axis.y.autoMin && axis.y.min > exVal.tyn) {
                        clipRect.enable = true;
                        clipRect.bottom = 0;
                    }
                }
                ...
                TrendlineRender.playHAnimation(duration, easing, animationSet, cBounds, paths, clipRect);
            };

            TrendlineRender.playHAnimation = function (duration, easing, animationSet, cBounds, paths, clipRect) {
            ...
            TrendlineRender.renderSingleTrendLine = function (series, seriesStyle, seriesHoverStyle, axis, hole, fieldsAniPathAttr, animation, seriesTransition, index, cBounds, canvas, paths, shadowPaths, animationSet, aniPathsAttr, wijCSS, seriesEles, inverted) {
                ...
                TrendlineRender.renderPath(canvas, cBounds, lineSeries, paths, shadowPaths, seriesHoverStyle, animationSet, pathArr, aniPathsAttr, initAniPath, lineStyle, index, wijCSS, axis);
            ...
            TrendlineRender.prototype.renderTrendLineChart = function (options, aniPathsAttr, fieldsAniPathAttr, paths, shadowPaths, animationSet, linesStyle, seriesEles) {
                var wijCSS = options.wijCSS, cBounds = options.bounds, canvas = options.canvas, axis = options.axis, ani = options.animation, hole = options.hole, seTrans = options.seriesTransition, needAnimated = false, linesSeries = options.seriesList, linesSeriesStyles = options.seriesStyles, linesHoverStyles = options.seriesHoverStyles;

                $.each(linesSeries, function (k, trendlineSeries) {
                    TrendlineRender.renderSingleTrendLine(trendlineSeries, linesSeriesStyles[k], linesHoverStyles[k], axis, hole, fieldsAniPathAttr, ani, seTrans, k, cBounds, canvas, paths, shadowPaths, animationSet, aniPathsAttr, wijCSS, seriesEles);
                });
            };

            TrendlineRender.renderPath = function (canvas, bounds, lineSeries, paths, shadowPaths, lineHoverStyle, animationSet, pathArr, aniPathsAttr, initAniPath, lineStyle, pathIdx, wijCSS, axis, inverted) {
        ...
        var wijchartcore = (function (_super) {
            ...
            wijchartcore.prototype._extendArrayFromBase = function (optionName) {
                var result = $.extend(true, {}, wijchartcore.prototype.options[optionName], this.options[optionName]), newArr = [];
            ...
            * @remarks Default exported file type is png, possible types are: jpg, png, gif, bmp, tiff, pfd.
            ...
            */
            wijchartcore.prototype.exportChart = function (fileName, type, pdfSettings, serviceUrl) {
            };
            ...
            */
            ...
            wijchartcore.prototype._paintLegendIcon = function (x, y, width, height, style, legendIndex, seriesIndex, legendCss, series, leg) {
                ...
                if (self._isTrendline(series)) {
                    icon = TrendlineRender.paintLegendIcon(x, y, width, height, style, self.canvas, self.options.legend.size, self.legendIcons, legendIndex, seriesIndex, legendCss);
                } else {
                    icon = self.canvas.rect(x, y, width, height);
            ...
            wijchartcore.prototype._paintLegend = function () {
                ...
                if (self.widgetName === "wijcompositechart") {
                    ...
                    $.each(tempSeriesList, function (i, series) {
                        ...
                        } else if (series.type === "sharedPie") {
                            if (series.sharedPieLegends) {
                                totalLengedLength += series.sharedPieLegends.length;
                            }
                        } else {
                            totalLengedLength++;
                        }
                ...
                $.each(tempSeriesList, function (i, series) {
                    ...
                    function drawSeriesLegend(series) {
                        ...
                        if (series.legendEntry && series.display !== "exclude") {
                            ...
                            chtStyle = chartStyle;
                            if (!(parseInt(chartStyle["stroke-width"]) === 0)) {
                                chtStyle = $.extend(chtStyle, { "stroke-width": 1 });
                            }
                    ...
                    if (series.type === "pie" && series.legendEntry) {
                        ...
                        if (series.sharedPieLegends) {
                            $.each(series.sharedPieLegends, function (j, data) {
                                data = $.extend({
                ...
                $.each(self.legends, function (idx, leg) {
                    ...
                    var bBox = leg.wijGetBBox(), icon, strokeWidth = chtStyle["stroke-width"] || 0, x = left + index * (iconWidth + strokeWidth * 2 + maxWidth + legendMargin) + (index + 1) * textMargin.left + index * textMargin.right, y = top + offsetY + bBox.height / 2 + textMargin.top + maxHeight / 2 - bBox.height / 2, iconY = y - iconHeight / 2 - strokeWidth, chtStyle, legCover, leg0 = leg;
