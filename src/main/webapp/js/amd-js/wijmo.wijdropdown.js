    ...
    (function (dropdown) {
        ...
        var wijdropdown = (function (_super) {
            ...
            wijdropdown.prototype._createSelect = function () {
                ...
                var self = this, o = self.options, wijCSS = self.options.wijCSS, ele = self.element, width = ele.width(), eleWidth = width, selectWrap = ele.wrap("<div></div>").parent().addClass(wijCSS.helperHidden), container = selectWrap.wrap("<div></div>").parent().attr("role", "select").addClass(wijCSS.wijdropdown).addClass(wijCSS.widget).addClass(wijCSS.stateDefault).addClass(wijCSS.cornerAll).addClass(wijCSS.helperClearFix), label = $("<label></label>").addClass(wijCSS.wijdropdownLabel).addClass(wijCSS.cornerAll).attr("id", ele.get(0).id + "_select"), rightTrigger = $("<div></div>").addClass(wijCSS.wijdropdownTrigger).addClass(wijCSS.stateDefault).addClass(wijCSS.cornerRight), labelWrap = $("<a href=\"#\"></a>"), listContainer = $("<div>").addClass(wijCSS.wijmodropdown), list = $("<ul></ul>").addClass(wijCSS.wijdropdownList).addClass(wijCSS.content).addClass(wijCSS.widget).addClass(wijCSS.cornerAll).addClass(wijCSS.helperReset).appendTo(listContainer);
                ...
                if (ele.get(0).tabIndex.toString() !== "") {
            ...
            wijdropdown.prototype._buildList = function (list, listContainer, eleWidth) {
                ...
                if (listContainer.data("wijmo-wijsuperpanel")) {
                    ...
                    self.superpanel = listContainer.wijsuperpanel({ "unfocus": self.options.unfocus }).data("wijmo-wijsuperpanel");
                }
                if ($.fn.bgiframe) {
            ...
            wijdropdown.prototype._show = function () {
                ...
                if (showingAnimation) {
                    ...
                    listContainer.show(showingAnimation.effect, showingAnimation.options, showingAnimation.speed, function () {
                        self._initActiveItem();
                    });
                } else {
                    listContainer.show();
            ...
            wijdropdown.prototype._hide = function () {
                ...
                if (hidingAnimation) {
                    ...
                    listContainer.hide(hidingAnimation.effect, hidingAnimation.options, hidingAnimation.speed, function () {
            ...
            wijdropdown.prototype._setValueToEle = function () {
                var self = this, ele = self.element, oldSelectedItem = ele.find(":selected"), oldSelectedIndex = $('option', ele).index(oldSelectedItem), selectedIndex = self._selectedIndex;
        ...
        var wijdropdown_options = (function () {
            function wijdropdown_options() {
                ...
                */
                this.showingAnimation = { effect: "blind" };
                ...
                */
                this.hidingAnimation = { effect: "blind" };
        ...
        ;
        wijdropdown.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, new wijdropdown_options());
