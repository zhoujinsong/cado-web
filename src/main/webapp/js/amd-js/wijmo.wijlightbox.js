...
(function (wijmo) {
    ...
    */
    (function (_lightbox) {
        ...
        var wijlightbox = (function (_super) {
            ...
            wijlightbox.prototype._create = function () {
                ...
                this.frame = $('<div></div>').addClass([
                    'wijmo-wijlightbox-frame', o.wijCSS.content,
                    o.wijCSS.cornerAll, o.wijCSS.helperClearFix].join(' ')).appendTo(this.container);
            ...
            */
            ...
            wijlightbox.prototype._getFragmentId = function (a) {
                var fragmentId = /^#.+/, href = $(a).attr('href') || "", hrefBase = href.split('#')[0], baseEl = $('base')[0];
            ...
            wijlightbox.prototype._parseLink = function (a) {
                var self = this, o = this.options, $a = $(a), opt = { href: undefined, player: undefined, img: undefined, gallery: undefined }, rel, $img, gallery, index, props = [
                    'href', 'player', 'title', 'alt',
            ...
            wijlightbox.prototype._createText = function (updateOnly) {
                ...
                if (o.textPosition === 'inside') {
                    ...
                    $text.addClass([o.wijCSS.content, o.wijCSS.cornerAll, o.wijCSS.helperClearFix].join(' ')).width(this.frame.width()).appendTo(this.container);
                    ...
                    if (o.textPosition === 'titleOverlay') {
                        $text.bind({
                            'mouseenter': function () {
                                ...
                                var totalHeight = $title.outerHeight(true) + $detail.outerHeight(true), animOption = $.extend({}, self._defaults.textShowOption, o.textShowOption, { queue: false });
                                $mask.animate({
                                    height: totalHeight
                                }, animOption);
                            ...
                            'mouseleave': function () {
                                ...
                                var titleHeight = $title.height(), animOption = $.extend({}, self._defaults.textHideOption, o.textHideOption, { queue: false });
                                $mask.animate({
                                    height: titleHeight
                                }, animOption);
                ...
                $.each(buttons, function (index, name) {
                    ...
                    if (name === 'play' && !this.playBtn) {
                        self.playBtn = $("<a href='#'/>").addClass([
                            ...
                            wijCSS.stateDefault,
                            wijCSS.cornerAll].join(' ')).append("<span class='" + wijCSS.icon + " " + wijCSS.iconPlay + "'></span>").hide().click(function () {
                            if (self.isPlaying()) {
                                self.stop();
                            } else {
                        ...
                        self.stopBtn = $("<a href='#'/>").addClass([
                            ...
                            wijCSS.cornerAll].join(' ')).append("<span class='" + wijCSS.icon + " " + wijCSS.iconStop + "'></span>").hide().click(function () {
                            if (self.isPlaying()) {
                                self.stop();
                                self.show(0);
                            }
            ...
            wijlightbox.prototype._slideAnimate = function (o, el, done) {
                var props = ["position", "top", "bottom", "left", "right", "width", "height"], mode = $.setMode(el, o.mode || "show"), show = mode === "show", direction = o.direction || "left", ref = (direction === "up" || direction === "down") ? "top" : "left", positiveMotion = (direction === "up" || direction === "left"), distance, animation = {};
            ...
            wijlightbox.prototype._slideTo = function (prevPlayer, player, animation, next, complete) {
                ...
                if (curImage.parent().is('.wijmo-wijlightbox-aniwrapper')) {
                    ...
                    wrapper = $.createWrapper(curImage).css({ overflow: 'hidden' });
            ...
            wijlightbox.prototype._moveTo = function (rect, animation, complete) {
                ...
                var hd = animation.duration / 2, animated = animation.animated, movePos = self._isOpen(), pos1 = animated === 'wh' ? { left: rect.left, top: undefined } : { top: rect.top }, pos2 = animated === 'wh' ? { top: rect.top, left: undefined } : { left: rect.left }, size1 = animated === 'wh' ? { width: rect.width, height: undefined } : { height: rect.height }, size2 = animated === 'wh' ? { height: rect.height, width: undefined } : { width: rect.width };
            ...
            */
            ...
            wijlightbox.prototype._resize = function (complete) {
                var self = this, rect = $.extend({}, self.container.offset(), {
                    width: self.frame.width(),
                    height: self.frame.height()
                });
            ...
            */
            ...
            wijlightbox.prototype._startTimer = function () {
                ...
                if (o.showTimer === true) {
                    ...
                    self.timerMeter.animate({ width: width }, o.delay, null, function () {
            ...
            wijlightbox.prototype._position = function () {
                var o = this.options, myAt = [], offset = [0, 0], isVisible, pos = o.position;
                ...
                this.container.css({ top: 0, left: 0 }).position(pos);
                if (!isVisible) {
                    this.container.hide();
                }
            };
            ...
            wijlightbox.prototype._getPlayerName = function (url) {
                ...
                var imgExt = ["bmp", "gif", "jpg", "jpeg", "png"], swfExt = ["swf"], flvExt = ["flv", "m4v"], qtExt = ["dv", "mov", "moov", "movie", "mp4", "avi", "mpg", "mpeg"], wmpExt = ["asf", "avi", "mpg", "mpeg", "wm", "wmv"], html5Video = ["mp4", "mpg", "mpeg", "ogg", "ogv"], ext = this._getExt(url);
        ...
        _lightbox.wijlightbox = wijlightbox;
                ...
                */
                this.position = {
                    ...
                    using: function (pos) {
                        var t = $(this);
                        t.css(pos);
                        var topOffset = t.offset().top;
                        if (topOffset < 0) {
                            t.css('top', pos.top - topOffset);
                        }
                    }
                };
        ...
        wijlightbox.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, new wijlightbox_options());
        ...
        $.extend($.wijmo.wijlightbox, {
            img: function (lightbox, item, onload, onerror) {
                ...
                var self = this;
                $('<img></img>').css({ width: 'auto', height: 'auto', display: 'none' }).appendTo(document.body).bind({
                    'load': function () {
                        var img = $(this);
