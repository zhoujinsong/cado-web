    ...
    */
    (function (filter) {
        ...
        var wijfilter = (function (_super) {
            ...
            */
            wijfilter.prototype.triggerClose = function (settingsChanged) {
                this._trigger("close", null, settingsChanged ? { sortDirection: this.options.sortDirection, filterValue: this.options.filterValue, filterOperator: this.options.filterOperator } : null);
            };
            ...
            wijfilter.prototype._onFilterAreaChanged = function (e, args) {
                ...
                this.options.filterValue = $.map(this._elements.$filterValuesDdl, function (o, i) {
                    ...
                    return (idx >= 0) ? [[self._distinctDataValues[idx]]] : [[$item.val() || undefined]];
                });
            };
            ...
            wijfilter.prototype._resetFilterArea = function () {
                ...
                // reset values
                this._elements.$filterValuesDdl.val("").wijcombobox("option", "selectedIndex", -1);
            };

            wijfilter.prototype._resetCheckboxes = function () {
                this._elements.$checkboxes.closest("li").show().end().prop("checked", false).wijcheckbox("refresh");
            ...
            wijfilter.prototype._attachElements = function (dataValues, availableOperators) {
                ...
                // -- create widgets --
                this._elements.$headerCloseBtn.button({ text: false, icons: { primary: wijCSS.iconClose } }).click(function () {
                    self.triggerClose(false);
                });

                // sort buttons
                this._elements.$ascendingBtn.button({ text: true, icons: { primary: 'ui-icon-arrowthickstop-1-n' } }).click($.proxy(self._onAscendingButtonClick, this));

                this._elements.$descendingBtn.button({ text: true, icons: { primary: 'ui-icon-arrowthickstop-1-s' } }).click($.proxy(self._onDescendingButtonClick, this));
                ...
                foo = $.map(dataValues, function (item, i) {
                    return { label: self._getDisplayValue(item) + "", value: self._getDisplayValue(item) };
                });
                ...
                if (allEquals_Or && fv.length > 1) {
                    $.each(fv, function (i, value) {
                        ...
                        if ((idx = $.inArray(value, self._distinctDataValuesCI)) >= 0) {
                            $(self._elements.$checkboxes[idx + 1]).prop("checked", true).wijcheckbox("refresh");
                        }
                    });
                } else {
        ...
        wijfilter.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, new wijfilter_options());
